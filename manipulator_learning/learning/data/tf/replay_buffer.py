"""Implementation of a local replay buffer.
   Originally based on https://github.com/google-research/google-research/blob/master/dac/replay_buffer.py"""
...
class ReplayBuffer(object):
    ...
    def sample_as_timestep(self, batch_size=100):
        if batch_size > self.__len__():
            time_step = []
            num_samp_req = int(np.ceil(batch_size / self.__len__()))
            for i in range(num_samp_req):
                time_step.extend(self.sample(self.__len__()))
            time_step = time_step[:batch_size]
        else:
            time_step = self.sample(batch_size)
        return TimeStep(*zip(*time_step))

    def get_data_numpy(self):
        """
        Return all data as separate numpy arrays
        :return:
        """
        data = TimeStep(*zip(*self._buffer))
        ret = []
        for field in data:
            ret.append(np.stack(field).astype('float32'))
        return ret
